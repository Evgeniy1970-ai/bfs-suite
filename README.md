...
- 🚨 **Escalation system** — triggers on low similarity score or explicit user request, notifies via Telegram
...
| **WF2 — Query** | Webhook → escalation check → AI router → Legal / HR / Support agent → RAG search → response → logging |
...
| **WF5 — Escalation** | Triggered by WF2 on low similarity or user request → sends Telegram alert → inserts record to `escalations` |
...
On escalation:
- Telegram notification is sent with session details, agent type, user question, and similarity score
- Record is inserted into the `escalations` table
- User receives a friendly message that a human will follow up
